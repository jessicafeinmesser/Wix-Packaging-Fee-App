  ...
  async handle(name, request) {
    ...
    return this.callsHandler.run(
      ...
      monitoringService.instrumentWithData({
        handler: async () => handler(request),
        data: {
          appId: "1a153335-1edc-454a-b559-25698f94deec",
          extensionType: "cli-api",
          extensionName: name
        }
      })
    );
...
var CallsHandler = class {
  ...
  constructor() {
    this.wixClient = createClient({
      host: {
        getMonitoringClient: monitoringService.createSentryCloudflareClient
      },
      auth: {
        getAuthHeaders: (host) => {
        ...
        getActiveToken: () => {
          ...
          return store.auth.getActiveToken?.();
        }
      }
    });
  ...
  async run(accessToken, instanceId, fn) {
    const auth = accessToken ? createAppStrategy({ accessToken }) : null;
    const elevatedAuth = instanceId ? createAppStrategy({ instanceId }) : null;
...
(function(Permissions2) {
  ...
  Permissions2["SiteMember"] = "SiteMember";
...
async function checkPermission(request, permission) {
  ...
  const roles = getMatchingRolesForPermission(permission);
  if (!roles.includes(subjectType)) {
    throw new Error("Insufficient permissions");
  }
}
function getMatchingRolesForPermission(permission) {
  switch (permission) {
...
function extractUrlParts(url) {
  ...
  if (parts) {
    const [, filename, methodname] = parts;
    if (filename && methodname) {
      return {
        filename,
        methodname: methodname.replace(".ajax", "")
      };
    }
  }
}
...
function createClient2(webMethods2) {
  return {
    async processRequest(request, devMode = false) {
      const parts = extractUrlParts(request.url);
      if (!parts) {
      ...
      const loadWebMethodFile = webMethods2[`/${parts.filename}`];
      try {
        if (!loadWebMethodFile) {
          throw new Error(`Error loading web module ${parts.filename}: Cannot find module '${parts.filename}'`);
        }
        const webMethod = (await loadWebMethodFile())[parts.methodname];
        if (!webMethod) {
          throw new Error(`Error loading function from web module ${parts.filename}: function '${parts.methodname}' not found`);
        }
...
var WebMethodHandler = class {
  ...
  async handle(request) {
    ...
    return this.callsHandler.run(
      accessToken,
      null,
      async () => this.webMethodsClient.processRequest(request, false)
    );
  }
};
var logError = (error) => {
...
async function initialize({
  ...
  return {
    async fetch(request) {
      ...
      const evaluationError = eventsHandler.evaluationError ?? servicePluginHandler.evaluationError;
      if (evaluationError) {
        return Response.json(
          {
            type: "BACKEND_EVAL_ERROR",
      ...
      if (endpointName === "service-plugins") {
        return servicePluginHandler.handle(request).catch((error) => {
          logError(error);
          return new Response(formatError(), { status: 500 });
        });
      }
      ...
      if (endpointName === "functions") {
        const response = await apiHandler.handle(pathName, request).then((result) => {
          if (result == null || result instanceof Response) {
            return result;
          }
          throw new Error(
            "You should always return a `new Response()` from an API function"
          );
        }).catch((error) => {
          logError(error);
          return new Response(formatError(), { status: 500 });
...
var apis = /* #__PURE__ */ Object.assign({"/src/backend/api/packaging-fee-api/api.ts": () => import('./assets/api.mjs')
...
var webMethods = /* #__PURE__ */ Object.assign({

});
var entry_default = monitoringService.withSentry(
  await initialize({
    apis,
    events,
    servicePlugins,
